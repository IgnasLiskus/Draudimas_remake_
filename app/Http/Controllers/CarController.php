...
class CarController extends Controller
{
    ...
    public function index()
    {
        ...
        if ($user->isAdmin() || $user->isViewer()) {
            ...
            // plain user sees only cars whose owner belongs to them
    ...
    public function create()
    {
        ...
        // admin sees all owners, others only see their own
        $owners = $user->isAdmin()
            ? Owner::all()
    ...
    public function store(Request $request)
    {
        ...
        // prevent non-admin from assigning car to someone else's owner
        if (!$user->isAdmin()) {
            $ownerBelongsToUser = Owner::where('id', $request->owner_id)
